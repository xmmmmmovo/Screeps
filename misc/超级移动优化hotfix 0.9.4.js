...
function addObTask(path, idx) {
  ...
  for (let obData of observers) {
    if (Game.map.getRoomLinearDistance(obData.roomName, roomName) <= 10) {
      obData.taskQueue.push({ path, idx, roomName })
      break
    }
  }
}
  ...
  for (let obData of observers) {
    ...
    while (queue.length) {
      ...
      if (ob) {
        ...
        obTimer[Game.time + 1].push({ path: task.path, idx: task.idx, roomName }) // idx位置无direction
      } else {
...
module.exports = {
  ...
  deletePathInRoom(roomName) {
    ...
    if (splited && splited.length == 5) {
      ...
      let fromalCentralPos = formalize({ x: 25, y: 25, roomName })
