Creep.prototype.customMove = function (target, range = 1, ignoreCreep = true) {
  ...
  if (next) {
    if (Math.abs(curr.x - next.x) <= 1 && Math.abs(curr.y - next.y) <= 1) {
      ...
      if (!obstacle.length) obstacle = creep.room.lookForAt(LOOK_POWER_CREEPS, next.x, next.y)
      if (obstacle.length) {
        if (creep.memory.state == obstacle[0].memory.state || obstacle[0].memory.state == 'Harvest') {
