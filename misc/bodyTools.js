...
class bodyTools {
   ...
   * @memberof bodyTools
   */
  static getNum(body, bodypartNameList) {
    ...
    if (bodypartNameList) {
      for (let index0 = 0; index0 < compiledElementList.length; index0 += 3) {
        if (bodypartNameList.findIndex(val => bodyAbbreviation[compiledElementList[index0 + 1]] === val) !== -1) {
...
bodyTools.regExp = /([mwcarhit])([0-9]+)/g
bodyTools.mulRegExp = /([*])([0-9]+)/g
bodyTools.checkRegExp = /^([mwcarhit*][0-9]+)*$/
