...
global.roomResSvg = (res, allCnt) => {
  ...
  let svgs = r
    .map(e => {
      if (e[0] == 'empty') return
      let t = `<rect x="${(left / allCnt) * len}" width="${(e[1] / allCnt) * len}" height="8" fill="${
        RES_COLOR_MAP[e[0]]
      }"/>`
      left += e[1]
      return t
    ...
    .join('')
  return `<svg width="${len}px" height="8px"><rect width="500" height="10" fill="black"/>${svgs}</svg> ${_.padLeft(
    left,
    8
  )}/${_.padLeft(allCnt, 8)} = ${_.padRight(((left / allCnt) * 100).toFixed(2), 5)}%`
}
...
let pro = {
  ...
  showAllRes() {
    ...
    let roomResAll = rooms
      .map(e => [e.name, pro.getStorageTerminalRes(e)])
      .reduce((map, entry) => {
        map[entry[0]] = entry[1]
        return map
      }, {})
